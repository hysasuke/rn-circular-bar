...
const ProgresBar: React.FC<ProgressBarProps> = ({
  ...
  const strokeDashoffset = interpolateNode(animate, {
    inputRange: [0, 1],
    outputRange: [circumference, Math.round(circumference * (1 - value * 0.01))]
  });

  const text = interpolateNode(animate, {
    inputRange: [0, 1],
    ...
    () => [
      call([text], ([text]) => {
        ...
        inputRef?.current?.setNativeProps({
          text: `${text?.toString()}${showPercentSymbol ? "%" : ""}`
        });
      })
    ],
    [text]
  ...
  return (
    <View style={{ width: viewBoxDimension, height: viewBoxDimension }}>
      ...
      >
        <G
          rotation="-90"
          origin={`${viewBoxDimension / 2}, ${viewBoxDimension / 2}`}
        >
          <Circle
            cx="50%"
            cy="50%"
      ...
      >
        ...
        />
        {label && (
          <Text style={{ color, fontWeight: "bold", fontSize: labelFontSize }}>
            {label}
          </Text>
        )}
      </View>
